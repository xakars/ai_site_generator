...
Фронтенд разработан и поддерживается отдельной командой. Для него развёрнут свой репозиторий, в рамках которого происходит тестирование и деплой.
В рамках разработки бэкенда используем уже готовый скомпилированный фронтенд для локальных тестов, который нам предоставляет команда фронтенда.
Файлы фронтенда следует положить в папку `src/static` 
